...
export default function HealthTips() {
  ...
  const [favourites, setFavourites] = useState(() => {
    try { return JSON.parse(localStorage.getItem('fav_cards') || '[]') } catch { return [] }
  })
  ...
  async function fetchCards() {
    try {
      const res = await databases.listDocuments(DB_ID, COLS.flashcards, [
        Query.equal('is_active', true),
        Query.orderAsc('display_order'),
        Query.limit(100)
      ])
      const filtered = res.documents.filter(c =>
        c.clinic_type === tab || c.clinic_type === 'both'
      )
      setCards(filtered)
  ...
  return (
    <div className="page" style={{ background: '#f8f9fa', minHeight: '100vh', paddingBottom: 20 }}>
      <div className="header" style={{ background: 'white', boxShadow: '0 1px 6px rgba(0,0,0,0.06)' }}>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 700, color: '#333' }}>Health Tips</h2>
      </div>
      ...
      <div style={{ display: 'flex', background: 'white', borderBottom: '1px solid #eee' }}>
        {['homeopathy', 'leanlife'].map(t => (
          <button key={t} onClick={() => { setTab(t); setSearch('') }} style={{
            ...
            background: tab === t ? (t === 'homeopathy' ? '#E3F2FD' : '#E8F5E9') : 'white',
            color: tab === t ? (t === 'homeopathy' ? '#1565C0' : '#2E7D32') : '#888',
            fontWeight: tab === t ? 700 : 400, fontSize: 14, cursor: 'pointer',
            borderBottom: `3px solid ${tab === t ? (t === 'homeopathy' ? '#1565C0' : '#2E7D32') : 'transparent'}`
          }}>
      ...
      <div style={{ padding: '12px 16px' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <input className="input" placeholder="🔍 Search health tips..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{ flex: 1, borderColor: theme.accent + '40' }} />
          <button onClick={() => setShowFavs(!showFavs)} style={{
            padding: '0 14px', border: `2px solid ${showFavs ? '#E91E63' : '#eee'}`,
            borderRadius: 12, background: showFavs ? '#FCE4EC' : 'white',
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', color: '#aaa' }}>
            ...
            <p>{showFavs ? 'No favourites saved yet' : 'No health tips found'}</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {filtered.map(card => (
              <div key={card.$id} style={{ perspective: 1000, height: 220 }}>
                <div onClick={() => toggleFlip(card.$id)} style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ padding: '10px 12px' }}>
                      <p style={{ fontSize: 13, fontWeight: 700, color: theme.accent, marginBottom: 4, lineHeight: 1.3 }}>
                        {card.title}
                      </p>
                      <p style={{ fontSize: 11, color: '#777', lineHeight: 1.4 }}>
                        {card.front_description?.substring(0, 60)}{card.front_description?.length > 60 ? '...' : ''}
                      </p>
                    </div>
                    ...
                    <div style={{ position: 'absolute', bottom: 8, right: 10, fontSize: 10, color: '#bbb' }}>
                      Tap to flip →
                    </div>
                  </div>
                  ...
                  <div style={{
                    position: 'absolute', width: '100%', height: '100%',
                    backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden',
                    borderRadius: 16, background: theme.bg,
                    transform: 'rotateY(180deg)',
                    boxShadow: '0 2px 12px rgba(0,0,0,0.08)',
                    padding: 14, display: 'flex', flexDirection: 'column'
                  }}>
                    <p style={{ fontSize: 13, fontWeight: 800, color: theme.accent, marginBottom: 8 }}>
                      {card.title}
                    </p>
                    <p style={{ fontSize: 12, color: '#444', lineHeight: 1.5, flex: 1, overflowY: 'auto' }}>
                      {card.back_content}
                    </p>
                    <p style={{ fontSize: 10, color: '#999', marginTop: 8, textAlign: 'right' }}>← Tap to flip back</p>
                  </div>
                </div>
              </div>
            ))}
