...
export default function PatientLogin() {
  ...
  async function handleLogin() {
    if (!phone || !code) { setError('Please enter phone and access code'); return }
    setLoading(true)
    setError('')
    try {
      ...
      const matched = res.documents.filter(p =>
        p.clinic_type === clinicType || p.clinic_type === 'both'
      )
      ...
      if (matched.length === 1) {
        loginPatient({ ...matched[0], clinicType, clinicName })
        navigate('/dashboard')
      } else {
        setPatients(matched)
        setShowSelect(true)
  ...
  return (
    <div className="page" style={{ background: theme.bg, minHeight: '100vh' }}>
      <div className="header" style={{ background: theme.bg }}>
        ...
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: theme.accent }}>{clinicName}</h2>
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{
            width: 90, height: 90, borderRadius: 24, background: 'white',
            ...
            <img
              ...
              onError={e => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = `<span style="font-size:40px">${isHomeo ? '🌿' : '🥗'}</span>` }}
            />
          </div>
          <h3 style={{ fontSize: 22, fontWeight: 800, color: theme.accent }}>{clinicName}</h3>
          <p style={{ color: '#888', fontSize: 14, marginTop: 4 }}>Enter your credentials to continue</p>
        </div>
        ...
        {!showSelect ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button onClick={handleLogin} disabled={loading} style={{
              ...
              marginTop: 8, boxShadow: `0 4px 16px ${theme.btn}40`
            }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {patients.map(p => (
                <button key={p.$id} onClick={() => selectPatient(p)} style={{
                  padding: '16px', border: `2px solid ${theme.card}`,
            ...
            <button onClick={() => setShowSelect(false)} style={{
